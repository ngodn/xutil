package id.abtech.xlinxutil.xray.util
...
import id.abtech.xlinxutil.xray.dto.AngConfig.VmessBean
import id.abtech.xlinxutil.xray.dto.V2rayConfig
...
import id.abtech.xlinxutil.xray.service.V2RayVpnService
...
//            }
            ...
            if (app.defaultDPreference.getPrefBoolean(V2RayVpnService.PREF_LOCAL_DNS_ENABLED, false)) {
    ...
    private fun inbounds(vmess: VmessBean, v2rayConfig: V2rayConfig, app: AngApplication): Boolean {
        try {
            v2rayConfig.inbounds.forEach { curInbound ->
                if (!app.defaultDPreference.getPrefBoolean(V2RayVpnService.PREF_PROXY_SHARING, false)) {
                    //bind all inbounds to localhost if the user requests
                    curInbound.listen = "127.0.0.1"
                }
            }
            v2rayConfig.inbounds[0].port = 10808
//            val socksPort = Utils.parseInt(app.defaultDPreference.getPrefString(V2RayVpnService.PREF_SOCKS_PORT, "10808"))
//            val lanconnPort = Utils.parseInt(app.defaultDPreference.getPrefString(V2RayVpnService.PREF_HTTP_PORT, ""))
...
//            }
            v2rayConfig.inbounds[0].sniffing?.enabled = app.defaultDPreference.getPrefBoolean(V2RayVpnService.PREF_SNIFFING_ENABLED, true)
        ...
        try {
            ...
            when (configType) {
                EConfigType.VMESS -> {
                    ...
                    //Mux
                    val muxEnabled = false//app.defaultDPreference.getPrefBoolean(V2RayVpnService.PREF_MUX_ENABLED, false)
...
//                        }
    ...
    private fun routing(vmess: VmessBean, v2rayConfig: V2rayConfig, app: AngApplication): Boolean {
        try {
            ...
            v2rayConfig.routing.domainStrategy = app.defaultDPreference.getPrefString(V2RayVpnService.PREF_ROUTING_DOMAIN_STRATEGY, "IPIfNonMatch")
            val routingMode = app.defaultDPreference.getPrefString(V2RayVpnService.PREF_ROUTING_MODE, "0")
